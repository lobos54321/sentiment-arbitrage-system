...
export EVALUATOR_SNAPSHOT_REVIEW_HISTORY_HOURS="${EVALUATOR_SNAPSHOT_REVIEW_HISTORY_HOURS:-72}"
export EVALUATOR_SNAPSHOT_LONG_HISTORY_HOURS="${EVALUATOR_SNAPSHOT_LONG_HISTORY_HOURS:-720}"
export EVALUATOR_SNAPSHOT_SOURCE_BUSY_TIMEOUT_MS="${EVALUATOR_SNAPSHOT_SOURCE_BUSY_TIMEOUT_MS:-30000}"
export EVALUATOR_SNAPSHOT_REQUIRED_PAPER_JOURNAL_MODE="${EVALUATOR_SNAPSHOT_REQUIRED_PAPER_JOURNAL_MODE:-WAL}"
case "$EVALUATOR_SNAPSHOT_REQUIRED_PAPER_JOURNAL_MODE" in
  WAL) ;;
  *) export EVALUATOR_SNAPSHOT_REQUIRED_PAPER_JOURNAL_MODE=WAL ;;
esac
export EVALUATOR_SNAPSHOT_MAX_SOURCE_READ_LOCK_SEC="${EVALUATOR_SNAPSHOT_MAX_SOURCE_READ_LOCK_SEC:-900}"
case "$EVALUATOR_SNAPSHOT_MAX_SOURCE_READ_LOCK_SEC" in
  [1-9]|[1-9][0-9]|[1-8][0-9][0-9]|900) ;;
  *) export EVALUATOR_SNAPSHOT_MAX_SOURCE_READ_LOCK_SEC=900 ;;
esac
export EVALUATOR_SNAPSHOT_RESTART_DELAY_SEC="${EVALUATOR_SNAPSHOT_RESTART_DELAY_SEC:-60}"
export EVALUATOR_SNAPSHOT_FAILURE_RETRY_SEC="${EVALUATOR_SNAPSHOT_FAILURE_RETRY_SEC:-60}"
  ...
  (
    ...
    while true; do
      ...
      PYTHONUNBUFFERED=1 python3 scripts/cross_db_evaluator_snapshot.py \
        ...
        --required-paper-journal-mode "$EVALUATOR_SNAPSHOT_REQUIRED_PAPER_JOURNAL_MODE" \
        --max-source-read-lock-sec "$EVALUATOR_SNAPSHOT_MAX_SOURCE_READ_LOCK_SEC" \
        --keep-previous 0 \
        --max-runs 0 \
        --interval-sec "${EVALUATOR_SNAPSHOT_INTERVAL_SEC:-21600}" \
        --failure-retry-sec "$EVALUATOR_SNAPSHOT_FAILURE_RETRY_SEC" \
