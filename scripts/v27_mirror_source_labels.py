...
def verify_source_label_mirror_parity(db_path, event_log_dir, *, since_id=None, until_id=None, limit=None, table="signal_features"):
    ...
    summary["duplicate_source_label_ids"] = sorted([source_label_id for source_label_id, count in mirrored_counts.items() if count > 1])
    summary["orphan_mirrored_source_label_ids"] = sorted(mirrored_id_set - db_id_set)
    try:
