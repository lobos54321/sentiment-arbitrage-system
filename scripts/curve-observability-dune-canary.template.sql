-- Pump.fun TradeEvent export for the curve observability canary.
-- Output is one row per TradeEvent joined to one signal window.
-- This is an observability/cost/coverage query only.
WITH signal_windows(window_id, token_ca, signal_ts, window_start_ts, window_end_ts, query_start_ts, query_end_ts) AS (
  VALUES
