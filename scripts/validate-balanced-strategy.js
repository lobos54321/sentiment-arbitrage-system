#!/usr/bin/env node
/**
 * BALANCED策略验证脚本 - 使用交叉验证测试BALANCED策略
 *
 * 方法: 时间分割验证
 * - 训练集: 较早的日志文件
 * - 测试集: 较晚的日志文件
...
console.log(`\n${'═'.repeat(70)}`);
console.log(`🔬 BALANCED策略验证测试`);
console.log(`${'═'.repeat(70)}`);
console.log(`\n策略参数:`);
console.log(`  - rejectDecayingSignal: true (排除DECAYING信号)`);
console.log(`  - requireSignalAccelerating: false`);
console.log(`  - smCountThreshold: ≥1`);
console.log(`  - baseScoreThreshold: ≥50`);
...
const testGoldPass = testGold.filter(balancedFilter);
const testNoisePass = testNoise.filter(balancedFilter);
...
// 与训练集对比
const trainGoldPass = trainGold.filter(balancedFilter);
const trainNoisePass = trainNoise.filter(balancedFilter);
...
// 列出测试集中通过的金狗
console.log(`\n${'═'.repeat(70)}`);
console.log(`🏆 测试集中通过BALANCED过滤的金狗`);
console.log(`${'═'.repeat(70)}`);
...
// 列出被错误过滤的金狗
console.log(`\n${'═'.repeat(70)}`);
console.log(`⚠️ 测试集中被BALANCED过滤掉的金狗 (漏掉的机会)`);
console.log(`${'═'.repeat(70)}`);

const missedGold = testGold.filter(g => !balancedFilter(g));
if (missedGold.length > 0) {
    missedGold.forEach(g => {
        const goldDogInfo = data.goldDogList.find(d => d.symbol === g.symbol);
        const gain = goldDogInfo ? goldDogInfo.gain : '?';
        const reason = g.signalTrendType === 'DECAYING' ? `信号=${g.signalTrendType}` :
                      g.smCount < 1 ? `SM=${g.smCount}` :
                      g.baseScore < 50 ? `Score=${g.baseScore}` : '未知';
...
// 与CONSERVATIVE对比总结
console.log(`\n${'═'.repeat(70)}`);
console.log(`📋 与CONSERVATIVE策略对比 (需同时参考CONSERVATIVE验证结果)`);
console.log(`${'═'.repeat(70)}`);
console.log(`  BALANCED策略特点:`);
console.log(`    - 金狗通过率更高 (预期85% vs 29%)`);
console.log(`    - 噪音过滤率更低 (预期19% vs 91%)`);
console.log(`    - 精确率更低 (预期6% vs 16%)`);
console.log(`    - 需要更多资金和更好的止损控制`);
