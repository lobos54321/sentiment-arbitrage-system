...
function replayForDelay({ dogs = [], duds = [], delayMin = 0, threshold = 0 }) {
  ...
  const dogRows = dogs.filter((row) => selectedByThreshold(row, threshold) && numeric(row[residualField]) != null);
  const dudRows = duds.filter((row) => selectedByThreshold(row, threshold) && numeric(row[residualField]) != null);
