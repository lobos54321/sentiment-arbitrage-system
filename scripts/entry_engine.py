...
SMART_ENTRY_ATH_FLAT_TINY_SCOUT_MIN_VOL_M5 = float(os.environ.get("SMART_ENTRY_ATH_FLAT_TINY_SCOUT_MIN_VOL_M5", "8000"))
SMART_ENTRY_ATH_FLAT_TINY_SCOUT_MIN_TX_M5 = int(os.environ.get("SMART_ENTRY_ATH_FLAT_TINY_SCOUT_MIN_TX_M5", "50"))
...
def _ath_flat_tiny_scout_ok(entry_readiness_policy, cached_trend, total_score=0):
    if not entry_readiness_policy:
        return False, {}
    if not _policy_allows(SMART_ENTRY_ATH_FLAT_TINY_SCOUT_MODE, entry_readiness_policy):
        return False, {}
    trend = cached_trend or {}
    buys = _safe_float(trend.get("buys_m5"), 0.0)
    sells = max(_safe_float(trend.get("sells_m5"), 1.0), 1.0)
    ...
    tx_m5 = buys + sells
    ok = (
        int(total_score or 0) >= SMART_ENTRY_ATH_FLAT_TINY_SCOUT_MIN_SCORE
        and pc_m5 >= SMART_ENTRY_ATH_FLAT_TINY_SCOUT_MIN_M5_PCT
        and bs_ratio >= SMART_ENTRY_ATH_FLAT_TINY_SCOUT_MIN_BS_RATIO
        and vol_m5 >= SMART_ENTRY_ATH_FLAT_TINY_SCOUT_MIN_VOL_M5
        and tx_m5 >= SMART_ENTRY_ATH_FLAT_TINY_SCOUT_MIN_TX_M5
    )
    return ok, {
        "entry_mode": SMART_ENTRY_ATH_FLAT_TINY_SCOUT_MODE,
        "total_score": total_score,
        "price_change_m5": pc_m5,
        "buy_sell_ratio": bs_ratio,
        "vol_m5": vol_m5,
        "tx_m5": tx_m5,
        "thresholds": {
            "total_score": SMART_ENTRY_ATH_FLAT_TINY_SCOUT_MIN_SCORE,
            "price_change_m5": SMART_ENTRY_ATH_FLAT_TINY_SCOUT_MIN_M5_PCT,
            "buy_sell_ratio": SMART_ENTRY_ATH_FLAT_TINY_SCOUT_MIN_BS_RATIO,
            "vol_m5": SMART_ENTRY_ATH_FLAT_TINY_SCOUT_MIN_VOL_M5,
            "tx_m5": SMART_ENTRY_ATH_FLAT_TINY_SCOUT_MIN_TX_M5,
        },
    }
...
def evaluate_smart_entry(token_ca, symbol='?', pool_address=None, entry_count=0,
    ...
    _ath_flat_tiny_requested = _policy_allows(
        SMART_ENTRY_ATH_FLAT_TINY_SCOUT_MODE,
        entry_readiness_policy,
    )
    if _chasing:
        if _explosive_direct_ok:
            ...
            _ath_flat_tiny_requested
            and bs_ratio >= SMART_ENTRY_ATH_FLAT_TINY_SCOUT_MIN_BS_RATIO
            and pc_m5 >= SMART_ENTRY_ATH_FLAT_TINY_SCOUT_MIN_M5_PCT
        ):
            log.info(
                f"[SmartEntry] 🧪 ATH_FLAT_TINY_SCOUT: bypassing chasing_top for paper-only scout "
                f"pc_m5={pc_m5:+.1f}% bs={bs_ratio:.2f} chase={_chase_reason}"
            )
        else:
    ...
    _ath_flat_tiny_ok, _ath_flat_tiny_detail = _ath_flat_tiny_scout_ok(
        entry_readiness_policy,
        cached_trend,
        total_score,
    )
    ...
    if _ath_flat_tiny_ok:
        node_detail = (
            f"node=ath_flat_structure_tiny_scout "
            f"score={total_score} bs={bs_ratio:.2f} rvol={rvol:.1f}x "
            f"pc_m5={pc_m5:+.1f}% vol_m5=${vol_m5:.0f} "
            f"tx_m5={_ath_flat_tiny_detail.get('tx_m5'):.0f} "
            f"armed=({detail_str})"
        )
        log.info(f"[SmartEntry] 🧪 ${symbol} ATH_FLAT_TINY_SCOUT at ${price:.10f}: {node_detail}")
        return True, SMART_ENTRY_ATH_FLAT_TINY_SCOUT_MODE, node_detail, price
