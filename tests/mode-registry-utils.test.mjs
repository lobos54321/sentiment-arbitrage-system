...
test('entry mode registry exposes governance tiers and blocked modes', () => {
  ...
  assert.equal(summary.by_tier.revival_canary, 4);
  ...
  assert.ok(!summary.paper_blocked_modes.includes('ath_no_kline_tiny_probe'));
  assert.ok(!summary.paper_blocked_modes.includes('ath_uncertainty_tiny_scout'));
  assert.ok(!summary.paper_blocked_modes.includes('lotto_not_ath_reclaim_tiny_probe'));
...
test('registry groups virtual not-ath watch separately from entry modes', () => {
  ...
  assert.ok(grouped.revival_canary.some((entry) => entry.mode === 'ath_no_kline_tiny_probe'));
  assert.ok(grouped.revival_canary.some((entry) => entry.mode === 'ath_uncertainty_tiny_scout'));
  assert.ok(grouped.revival_canary.some((entry) => entry.mode === 'lotto_not_ath_reclaim_tiny_probe'));
  assert.ok(grouped.revival_canary.some((entry) => entry.mode === 'lotto_low_liquidity_reclaim_tiny_probe'));
