...
def test_previous_v3_parallel_history_is_not_reused_by_v4_stream_spool():
    estimates = shared_stage_estimates(
        required_bytes={"paper_decision_events": 24576}
    )
    history = shared_stage_history(
    ...
    plan = snapshot_module.build_shared_stage_budget_plan(
        ...
        history=history,
        history_anchor=anchor,
        attempt_id="v4-stream-spool-attempt",
    )

    assert plan["accepted"] is True
    assert plan["history_used"] is True
    candidate = plan["targets"][
        snapshot_module.SHARED_STAGE_TARGET_CANDIDATE
    ]
    assert candidate["history_storage_compatible"] is True
    for target in snapshot_module.PARALLEL_PAPER_STAGE_TABLES:
        report = plan["targets"][target]
        assert report["storage_schema_version"] == (
...
def test_compressed_parallel_stage_streams_one_row_across_bounded_chunks(
    tmp_path,
):
    source = tmp_path / "large-row-source.db"
    stage_path = tmp_path / "large-row-stage.db"
    final_path = tmp_path / "large-row-final.db"
    now = time.time()
    blob_size = 64 * 1024**2 + 4096
    source_db = sqlite3.connect(source)
    source_db.executescript(
        "CREATE TABLE opportunity_events("
        "id INTEGER PRIMARY KEY,event_ts REAL NOT NULL,payload BLOB);"
        "CREATE INDEX idx_opportunity_events_recent "
        "ON opportunity_events(event_ts);"
    )
    source_db.execute(
        "INSERT INTO opportunity_events VALUES (?, ?, zeroblob(?))",
        (1, now - 10, blob_size),
    )
    source_db.commit()
    source_db.close()
    ...
    chunks = list(
        stage.execute(
            f"SELECT row_count,raw_size_bytes FROM "
            f"{snapshot_module.PARALLEL_PAPER_STAGE_CHUNK_TABLE} "
            "ORDER BY sequence"
        )
    )
    stage.close()
    ...
    assert report["rows_copied"] == 1
    assert report["stage_raw_size_bytes"] > 64 * 1024**2
    assert report["stage_chunk_count"] == len(chunks) > 16
    assert sum(int(chunk[0]) for chunk in chunks) == 1
    assert any(int(chunk[0]) == 0 for chunk in chunks)
    assert all(
        0 < int(chunk[1])
        <= snapshot_module.PARALLEL_PAPER_STAGE_CHUNK_TARGET_BYTES
        for chunk in chunks
    )
    ...
    restored = final.execute(
        "SELECT typeof(payload),length(payload),payload "
        "FROM opportunity_events"
    ).fetchone()
    final.close()

    expected_digest = hashlib.sha256()
    zero_block = b"\x00" * 1024**2
    for _ in range(blob_size // len(zero_block)):
        expected_digest.update(zero_block)
    expected_digest.update(b"\x00" * (blob_size % len(zero_block)))
    assert restored[0] == "blob"
    assert restored[1] == blob_size
    assert hashlib.sha256(restored[2]).hexdigest() == expected_digest.hexdigest()
    assert merged["rows_merged"] == 1
    assert merged["stage_chunk_integrity_passed"] is True
    assert merged["stage_row_digest_matched"] is True
    ...
    try:
        ...
        with pytest.raises(
            ...
            build_snapshot_bundle(
                ...
                max_output_gib=0.1,
                max_source_read_lock_sec=0.5,
                snapshot_id="20260101T010000Z-1234abd3",
            )
        assert partial.is_dir()
    finally:
        release.set()
...
def test_bundle_output_accounting_includes_manifest_and_has_no_side_files(tmp_path):
    ...
    assert not snapshot_module.snapshot_worker_owner_path(out).exists()
    assert str(out.resolve()) not in snapshot_module._WORKER_OWNER_LEASES
...
def test_interrupted_partial_cleanup_is_strictly_scoped(
    ...
    snapshot_module._acquire_snapshot_worker_lease(
        tmp_path / "evidence"
    )
...
def test_partial_cleanup_requires_exact_held_lease_identity(
    tmp_path,
    monkeypatch,
):
    ...
    partial.mkdir(parents=True)
    actual_lease = lease_identity_fixture(out_root)
    write_partial_owner_fixture(
        partial,
        lease_identity={
            "device": actual_lease["device"],
            "inode": actual_lease["inode"] + 10_000,
        },
    )
    monkeypatch.setattr(
        snapshot_module,
    ...
    with pytest.raises(
        snapshot_module.SnapshotWorkerOwnerInvalidError,
        match="partial_owner_lease_mismatch",
    ):
...
def test_partial_cleanup_detects_late_directory_before_any_delete(
    tmp_path,
    monkeypatch,
):
    out_root = tmp_path / "evidence"
    ...
    attributed = snapshots / ".20260101T010000Z-abcdef12.partial"
    late = snapshots / ".20260101T020000Z-abcdef13.partial"
    attributed.mkdir(parents=True)
    write_partial_owner_fixture(attributed)
    created_late = False

    def create_late_partial_then_report_exit(_pid):
        nonlocal created_late
        if not created_late:
            late.mkdir(parents=True)
            (late / ".paper-decision-events-stage.db").write_bytes(b"live")
            created_late = True
    ...
    monkeypatch.setattr(
        snapshot_module,
        "snapshot_process_identity",
        create_late_partial_then_report_exit,
    )
    snapshot_module._acquire_snapshot_worker_lease(out_root)
...
def test_partial_cleanup_rejects_lease_path_replaced_after_lock(
    tmp_path,
    monkeypatch,
):
    out_root = tmp_path / "evidence"
...
def test_direct_build_marker_write_failure_leaves_no_visible_partial(
    tmp_path,
    monkeypatch,
):
    sources = create_sources(tmp_path)
    ...
    def fail_partial_marker(path, payload):
        if Path(path).name == snapshot_module.PARTIAL_OWNER_FILENAME:
            raise OSError("injected_partial_marker_write_failure")
        return original_atomic_json(path, payload)
    ...
    with pytest.raises(
        OSError,
        match="injected_partial_marker_write_failure",
    ):
        build_snapshot_bundle(
            sources=sources,
            out_root=str(out_root),
    ...
    snapshots = out_root / "snapshots"
    assert not any(
        snapshot_module.PARTIAL_SNAPSHOT_NAME_RE.fullmatch(path.name)
