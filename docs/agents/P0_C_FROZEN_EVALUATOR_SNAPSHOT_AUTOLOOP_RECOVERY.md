...
### 2026-08-10｜P0-C.8 收敛：`shared_stage_budget.v2` advisory 分配与高水位迭代

- PR #81 exact SHA `7b46dcd55c35231dc5157b68882c6cf89986d1c4` 已 fast-forward 到 `main` 并部署；生产 paper DB 继续为 `ok`、约 25.58GB，integrity marker 不存在，但生产仍没有 accepted manifest；
- PR #81 的生产 attempt 证明总 stage pool 约 12.48GB 足够，而 P9 固定 grant 约 4.49GB 不足：`paper_decision_events` 在约 293.21 秒触发 `SQLITE_FULL`。因此本轮删除所有运行时固定 percentage，继续保持 10GiB output cap、5GiB reserve、96h/840h evidence windows 与每个 pinned source view 300 秒 ceiling 不变；
- 新合同为 `shared_stage_budget.v2 / history_high_water_plus_advisory_source_demand`。DBSTAT、indexed row count 与最多 256 条 edge sample 只产生可复算的 **advisory allocation demand**；manifest 必须显式声明 `physical_upper_bound_claimed=false`。合法 SQLite record/overflow packing 可能使 4KiB stage 文件大于表级 DBSTAT 推导值，因此 advisory 不能被当作物理安全证明；
- 真正的磁盘安全边界只有两层：全局 stage cap 严格等于 `floor((free - output_cap - reserve) / 4096) * 4096`，并且每个 stage 文件都按其 grant 设置独立 `PRAGMA max_page_count`。所有 grants 必须精确求和到全局 cap；即使 advisory 偏小，也只能触发 `SQLITE_FULL` 后 fail-closed，不能突破总 cap 或侵占 reserve；
- 无历史时，每个 active target 的 hard baseline 仅为 12,288 bytes；上一轮 completed target 使用 `1.10 × high-water`，incomplete target 使用 `1.20 × high-water`，cap-hit target 使用 `max(previous high-water, previous grant) + 1 page`。只有通过双 SHA、inventory、cleanup、grant 与 high-water 校验的历史才能回灌；
- baseline 合计超过全局 cap 时，在 copy 前以 `shared_stage_capacity_insufficient` 拒绝。否则 residual pool 按整数 page、largest-remainder 算法确定性分配：存在 cap-hit 时只向 cap-hit targets 倾斜；没有 cap-hit 时覆盖全部 active targets；权重为 `max(4096, advisory demand, previous grant)`；
- 失败清理前 producer 记录每个已登记 stage DB 及 journal/WAL/SHM 的 logical/allocated high-water、grant、utilisation、copy completion、cap-hit 与 advisory miss；只有 partial 和所有 stage sidecar 均删除、且不存在 rogue hidden stage 文件时，证据才可成为下一轮历史；
- accepted evidence 允许 `actual_usage_bytes > advisory_required_bytes`，但必须满足 `actual_usage_bytes <= granted_cap_bytes`、所有 target actual 合计不超过 global cap、行数与冻结库一致、stage report 实际大小一致、cleanup 完成。`targets_exceeding_advisory` 与 `advisory_miss_count` 是可观察诊断，不是放宽安全边界；
- `plan_sha256` 锁定 inventory、advisory、history baseline、allocation weight、grant、borrow 与 global cap；`evidence_sha256` 另行锁定 actual/high-water/advisory miss/cap-hit/cleanup。Python 与 Node 共同使用 `json_sorted_float64_bits.v1` 并实际复算两个 SHA；top-level 与 `disk_preflight.shared_stage_budget` 必须完全相同；
- authoritative Python consumer 独立复算 attached-source DBSTAT accounting、indexed row count、advisory 公式、history baseline、cap-hit priority、weighted residual、每 target grant、actual 与 stage-file binding、optional inventory、global cap、reserve、双 SHA 和 cleanup；任何 `physical_upper_bound_claimed=true`、actual 超 grant、allocation 漂移或 advisory-miss inventory 伪造均 fail-closed；
- Dashboard 只输出 public-safe 的 advisory/baseline/weight/grant/actual/high-water/utilisation/advisory-miss 信息，不公开路径、SQL、payload 或 secret；它同样复算 allocation 与双 SHA，但 `consumer_ready` 仍只来自 authoritative Python preflight；
