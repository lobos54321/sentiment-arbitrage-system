...
### 2026-08-10｜P0-C.8 共享 Staging 预算 v2 实施
...
- 最新生产失败已把矛盾压缩为 P9 固定单表 cap：aggregate stage pool 约 12.48GB，P9 grant 约 4.49GB，`paper_decision_events` 在 293.21 秒附近触发 `SQLITE_FULL`；AutoLoop 继续 `blocked_evaluator_snapshot_required`，P0-D 继续锁定；
- 隔离分支 `p0c/shared-stage-budget-20260809` 实施 `shared_stage_budget.v2 / history_high_water_plus_advisory_source_demand`，删除所有运行时固定 percentage。总 stage cap 仍由 free space 减 10GiB output cap 与 5GiB reserve 后按 4096-byte page 向下对齐；96h/840h evidence windows 与 300 秒 source-view ceiling 均不放宽；
- DBSTAT、indexed row count 与 edge samples 只生成 advisory allocation demand；合同显式要求 `physical_upper_bound_claimed=false`。SQLite record/overflow packing 可能使目标 4KiB stage 文件大于任何紧致表级推导，因此 advisory miss 是诊断和下一轮 high-water 输入，不是安全证明失败；
- 真正安全边界是：所有 target grants 精确求和到 global cap，每个 SQLite stage 文件按 grant 设置独立 `max_page_count`，actual 必须不超过 grant，global cap 与 reserve 必须通过。advisory 偏小时只能 `SQLITE_FULL` fail-closed，不能突破磁盘边界；
- hard baseline 仅来自 minimum 与经双 SHA 验证的历史 high-water：无历史为 12,288 bytes；completed 为 1.10 倍 high-water；incomplete 为 1.20 倍；cap-hit 为 `max(high-water, previous grant) + 1 page`。baseline 合计超池时 copy 前拒绝；
- residual pool 使用整数 page largest-remainder 分配。存在 cap-hit 时只优先 cap-hit targets；否则覆盖全部 active targets；权重为 `max(4096, advisory demand, previous grant)`。optional path 缺失时不创建 stage、不保留 grant，其空间自动进入 active inventory 的 residual 分配；
- producer 在 partial 清理前记录 logical/allocated high-water、grant、actual、utilisation、advisory miss、copy-complete、cap-hit 与 rogue-stage inventory；cleanup、inventory、global plan 或双 SHA 任一无效都禁止历史回灌；
- authoritative consumer 独立复算 attached-source DBSTAT accounting、indexed row count、advisory 公式、history baseline、priority、weights、largest-remainder allocations、grants、actual-to-stage-size binding、optional inventory、global cap、reserve、plan/evidence SHA 与 cleanup；Dashboard 做同构 public-safe 复算，但 `consumer_ready` 仍只接受 Python authoritative preflight；
