  ...
  if (url.pathname === '/' || url.pathname === '/health' || url.pathname === '/ping') {
    ...
    const dimSql = {
      ...
      source_resonance_state: `
        COALESCE(
          json_extract(o.payload_json, '$.source_resonance_state'),
          (
            SELECT COALESCE(sr.cohort, 'level_' || CAST(sr.resonance_level AS TEXT), 'seen')
            FROM source_resonance_candidates sr
            WHERE sr.token_ca = v.token_ca AND sr.signal_ts BETWEEN v.signal_ts - 600 AND v.signal_ts + 600
            ORDER BY ABS(sr.signal_ts - v.signal_ts), COALESCE(sr.resonance_score, 0) DESC
            LIMIT 1
          ),
          'UNKNOWN'
        )`,
      source_quote_clean: `
        CASE
          WHEN json_extract(o.payload_json, '$.source_quote_clean_seen') IS NOT NULL
            THEN CASE WHEN COALESCE(json_extract(o.payload_json, '$.source_quote_clean_seen'), 0) THEN 'true' ELSE 'false' END
          WHEN EXISTS (
            SELECT 1 FROM source_resonance_candidates sr
            WHERE sr.token_ca = v.token_ca AND sr.signal_ts BETWEEN v.signal_ts - 600 AND v.signal_ts + 600
          )
            THEN CASE WHEN EXISTS (
              SELECT 1 FROM source_resonance_candidates sr
              WHERE sr.token_ca = v.token_ca AND sr.signal_ts BETWEEN v.signal_ts - 600 AND v.signal_ts + 600
                AND (COALESCE(sr.quote_clean_seen, 0) = 1 OR COALESCE(sr.two_quote_clean_snapshots, 0) = 1 OR COALESCE(sr.entry_quote_success_seen, 0) = 1)
            ) THEN 'true' ELSE 'false' END
          ELSE 'unknown'
        END`,
      source_quote_executable_proxy: `
        CASE
          WHEN json_extract(o.payload_json, '$.source_quote_executable_proxy') IS NOT NULL
            THEN CASE WHEN COALESCE(json_extract(o.payload_json, '$.source_quote_executable_proxy'), 0) THEN 'true' ELSE 'false' END
          WHEN EXISTS (
            SELECT 1 FROM source_resonance_candidates sr
            WHERE sr.token_ca = v.token_ca AND sr.signal_ts BETWEEN v.signal_ts - 600 AND v.signal_ts + 600
          )
      ...
      markov_bucket: `
        COALESCE(
          json_extract(o.payload_json, '$.markov_bucket'),
          (
            SELECT lower(COALESCE(
              json_extract(r.payload_json, '$.gate.markov_bucket'),
              json_extract(r.payload_json, '$.gate.markovBucket'),
              json_extract(r.payload_json, '$.markov_reclaim_gate.markov_bucket'),
              json_extract(r.payload_json, '$.markov_reclaim_gate.markovBucket'),
              json_extract(r.payload_json, '$.markovReclaimGate.markov_bucket'),
              json_extract(r.payload_json, '$.markovReclaimGate.markovBucket'),
              json_extract(r.payload_json, '$.markov_reclaim_forecast.gate.markov_bucket'),
              json_extract(r.payload_json, '$.markov_reclaim_forecast.gate.markovBucket'),
              json_extract(r.payload_json, '$.markovReclaimForecast.gate.markov_bucket'),
              json_extract(r.payload_json, '$.markovReclaimForecast.gate.markovBucket'),
              json_extract(r.payload_json, '$.lotto_markov_reclaim_forecast.gate.markov_bucket'),
              json_extract(r.payload_json, '$.lotto_markov_reclaim_forecast.gate.markovBucket'),
              json_extract(r.payload_json, '$.revival_canary.markov_bucket'),
              json_extract(r.payload_json, '$.revival_canary.markovBucket'),
              json_extract(r.payload_json, '$.learning_bypass.markov_bucket'),
              json_extract(r.payload_json, '$.learning_bypass.markovBucket'),
              json_extract(r.payload_json, '$.markov_bucket'),
              json_extract(r.payload_json, '$.markovBucket')
            ))
            FROM (
              SELECT signal_id, token_ca, COALESCE(signal_ts, event_ts, 0) AS runtime_ts, payload_json
              FROM paper_decision_events
              UNION ALL
              SELECT signal_id, token_ca, COALESCE(signal_ts, created_event_ts, baseline_ts, 0) AS runtime_ts, payload_json
              FROM paper_missed_signal_attribution
            ) r
            WHERE r.token_ca = v.token_ca AND r.runtime_ts BETWEEN v.signal_ts - 600 AND v.signal_ts + 600
            ORDER BY CASE WHEN r.signal_id = v.signal_id THEN 0 ELSE 1 END, ABS(r.runtime_ts - v.signal_ts)
            LIMIT 1
          ),
          'UNKNOWN'
        )`,
      lifecycle_profile: `
        COALESCE(
          json_extract(o.payload_json, '$.lifecycle_profile'),
          (
            SELECT
              CASE
                WHEN r.lifecycle_state IS NOT NULL AND r.entry_bias IS NOT NULL THEN upper(r.lifecycle_state) || ':' || upper(r.entry_bias)
                WHEN r.lifecycle_state IS NOT NULL THEN upper(r.lifecycle_state)
                WHEN r.entry_bias IS NOT NULL THEN upper(r.entry_bias)
                ELSE NULL
              END
            FROM (
              SELECT signal_id, token_ca, COALESCE(signal_ts, event_ts, 0) AS runtime_ts, lifecycle_state, entry_bias
              FROM paper_decision_events
              UNION ALL
              SELECT signal_id, token_ca, COALESCE(signal_ts, created_event_ts, baseline_ts, 0) AS runtime_ts, lifecycle_state, entry_bias
              FROM paper_missed_signal_attribution
            ) r
            WHERE r.token_ca = v.token_ca AND r.runtime_ts BETWEEN v.signal_ts - 600 AND v.signal_ts + 600
            ORDER BY CASE WHEN r.signal_id = v.signal_id THEN 0 ELSE 1 END, ABS(r.runtime_ts - v.signal_ts)
      ...
      lifecycle_state: `
        COALESCE(
          json_extract(o.payload_json, '$.lifecycle_state'),
          (
            SELECT upper(r.lifecycle_state)
            FROM (
              SELECT signal_id, token_ca, COALESCE(signal_ts, event_ts, 0) AS runtime_ts, lifecycle_state
              FROM paper_decision_events
              UNION ALL
              SELECT signal_id, token_ca, COALESCE(signal_ts, created_event_ts, baseline_ts, 0) AS runtime_ts, lifecycle_state
              FROM paper_missed_signal_attribution
            ) r
            WHERE r.token_ca = v.token_ca AND r.runtime_ts BETWEEN v.signal_ts - 600 AND v.signal_ts + 600
            ORDER BY CASE WHEN r.signal_id = v.signal_id THEN 0 ELSE 1 END, ABS(r.runtime_ts - v.signal_ts)
    ...
    if (!dimSql) {
      ...
      res.end(JSON.stringify({ error: 'unsupported_dimension', supported_dimensions: [
        ...
        'source_quote_executable_proxy', 'markov_bucket', 'lifecycle_profile',
        'lifecycle_state', 'market_cap_bucket', 'fbr_bucket', 'first3_mom_bucket',
      ] }));
      return;
    }
    let paperDb;
