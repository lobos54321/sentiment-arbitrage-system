...
function agentCaptureArtifactPaths() {
  ...
  return {
    ...
    handoff_json: join(getAgentHandoffsDir(), 'latest_codex_handoff.json'),
    ...
    capture_60_gap_report: join(latestDir, 'capture_60_gap_report.json'),
    capture_stage_metrics: join(latestDir, 'capture_stage_metrics.json'),
    context_coverage: join(latestDir, 'context_coverage_audit_24h.json'),
    context_dimension_eligibility: join(latestDir, 'context_dimension_eligibility.json'),
    ...
    pending_to_final_entry_audit: join(latestDir, 'pending_to_final_entry_audit.json'),
    final_entry_readiness_audit: join(latestDir, 'final_entry_readiness_audit.json'),
    ...
    strategy_memory_capture_validation: join(latestDir, 'strategy_memory_capture_validation.json'),
    shadow_candidate_improvement_queue: join(latestDir, 'shadow_candidate_improvement_queue.json'),
    capture_cross_validity: join(latestDir, 'capture_cross_validity_24h.json'),
    clean_dimension_2d_capture_cross: join(latestDir, 'clean_dimension_2d_capture_cross_24h.json'),
    quality_timing_reason_cross: join(latestDir, 'quality_timing_reason_cross_24h.json'),
    strategy_memory_reason_cross: join(latestDir, 'strategy_memory_reason_cross_24h.json'),
    ...
    markov_candidate_volume: join(latestDir, 'candidate_virtual_markov_candidate_volume_24h.json'),
    markov_candidate_quote: join(latestDir, 'candidate_virtual_markov_candidate_quote_24h.json'),
    ...
    oos_readiness_summary: join(latestDir, 'oos_readiness_summary.json'),
    pass_allow_60_oos_freeze_registry: join(latestDir, 'pass_allow_60_oos_freeze_registry.json'),
    pass_allow_60_post_freeze_oos_validation: join(latestDir, 'pass_allow_60_post_freeze_oos_validation.json'),
    capture_cross_oos_freeze_registry: join(latestDir, 'capture_cross_oos_freeze_registry.json'),
    capture_cross_post_freeze_oos_validation: join(latestDir, 'capture_cross_post_freeze_oos_validation.json'),
    p7_exit_policy_oos_validation: join(latestDir, 'p7_exit_policy_oos_validation.json'),
    phase3_goal_loop: join(latestDir, 'phase3_goal_loop.json'),
    phase3_goal_loop_summary: join(latestDir, 'phase3_goal_loop.md'),
    p7_paper_proposal_checkpoint: join(latestDir, 'p7_paper_proposal_checkpoint.json'),
    p7_paper_proposal_checkpoint_md: join(latestDir, 'p7_paper_proposal_checkpoint.md'),
    phase3_wide_net_paper_contract: join(latestDir, 'phase3_wide_net_paper_contract.json'),
    phase3_wide_net_paper_experiment_summary: join(latestDir, 'phase3_wide_net_paper_experiment_summary.json'),
    phase3_p7_human_review_packet: join(latestDir, 'phase3_p7_human_review_packet.json'),
    phase3_p7_human_review_packet_md: join(latestDir, 'phase3_p7_human_review_packet.md'),
    phase3_24h_path_observer_summary: join(latestDir, 'phase3_24h_path_observer_summary.json'),
    phase3_path_horizon_audit: join(latestDir, 'phase3_path_horizon_audit_24h.json'),
    p9_metric_predictiveness_ledger: join(latestDir, 'p9_metric_predictiveness_ledger.json'),
    influence_kol_shadow_source_plan: join(latestDir, 'influence_kol_shadow_source_plan.json'),
    influence_kol_shadow_features: join(latestDir, 'influence_kol_shadow_features_24h.json'),
    ...
    pending_stale_before_final_watch_validation: join(latestDir, 'pending_stale_before_final_watch_validation_24h.json'),
    runtime_health: join(latestDir, 'runtime_health_snapshot_24h.json'),
    pump_fun_shadow_observer_summary: join(latestDir, 'pump_fun_shadow_observer_summary.json'),
    pump_fun_shadow_source_comparison: join(latestDir, 'pump_fun_shadow_source_comparison_24h.json'),
    pump_fun_shadow_source_comparison_30d: join(latestDir, 'pump_fun_shadow_source_comparison_30d.json'),
    pump_fun_shadow_worker_status: join(latestDir, 'pump_fun_shadow_worker_status.json'),
...
function readAgentCaptureLoopRunnerStatus() {
  ...
  if (staleRunning && (status.running || !status.reconciled_at)) {
    ...
    normalized.reconciliation_note = status.reconciliation_note
      || 'runner status normalized because stored running pid is not active or exceeded stale age';
...
export function buildAgentLatestStatus(options = {}) {
  ...
  const status = {
    ...
    runner_status: {
      ...
      scheduler: runner.scheduler || null,
      exec_run_provenance: runner.exec_run_provenance ? {
...
function triggerAgentCaptureDiscoveryLoop(url, options = {}) {
  ...
  const baseStatus = {
    ...
    scheduler: {
      ...
      report_timeout_sec: reportTimeoutSec,
      test_timeout_sec: testTimeoutSec,
      oos_probe_hours: oosProbeHours,
      max_runs: 1,
    },
    exec_run_provenance: {
...
function buildAgentCaptureDiscoveryLatestSnapshot(options = {}) {
  ...
  if (includeReports) {
    payload.reports = {
      ...
      clean_dimension_2d_capture_cross: safeReadAgentJson(paths.clean_dimension_2d_capture_cross),
      quality_timing_reason_cross: safeReadAgentJson(paths.quality_timing_reason_cross),
      strategy_memory_reason_cross: safeReadAgentJson(paths.strategy_memory_reason_cross),
      ...
      markov_candidate_volume: safeReadAgentJson(paths.markov_candidate_volume),
      markov_candidate_quote: safeReadAgentJson(paths.markov_candidate_quote),
      ...
      pending_stale_before_final_watch_validation: safeReadAgentJson(paths.pending_stale_before_final_watch_validation),
...
export function readEvaluatorSnapshotWorkerHealth(options = {}) {
  ...
  const sharedStageAdvisoryEvidencePassed = (target, raw, minimum, advisory) => {
    const evidence = raw.advisory_evidence
      && typeof raw.advisory_evidence === 'object'
      && !Array.isArray(raw.advisory_evidence)
      ? raw.advisory_evidence
      : {};
    ...
    const rowFractionNumerator = nonnegativeInt(
      'source_row_fraction_numerator',
    );
    const rowFractionDenominator = nonnegativeInt(
    ...
    if (
      [
        ...
        maxPayloadBytes,
        cellUpperCount,
        rowFractionNumerator,
        rowFractionDenominator,
        tableScaledPhysical,
        tableRowOverhead,
        tableRootReserve,
        tableAdvisory,
      ...
      || evidence.advisory_schema_version !== 'sqlite_dbstat_advisory_demand.v1'
      || evidence.advisory_formula !== sharedStageAdvisoryFormula
      ...
      || rowFractionNumerator !== selectedRows
      || rowFractionDenominator !== sourceRowsUpper
    ) {
      return false;
    }
    ...
    if (expectsIndex) {
      if (
        raw.advisory_strategy !== 'dbstat_proportional_advisory_with_indexed_row_count'
        || typeof evidence.source_index_name !== 'string'
        || evidence.source_index_name.length === 0
      ...
      raw.advisory_strategy !== 'dbstat_full_btree_advisory_demand'
      || selectedRows !== sourceRowsUpper
    ...
    const expectedTableScaled = selectedRows === 0 || sourceRowsUpper === 0
      ? 0
      : ceilMultiplyDivide(physicalBytes, selectedRows, sourceRowsUpper);
    const expectedTableRowOverhead = selectedRows * 32;
    const expectedTableAdvisory = expectedTableScaled == null
      || !Number.isSafeInteger(expectedTableRowOverhead)
      ? null
      : roundSharedStagePage(
        Math.max(
          minimum,
          expectedTableScaled + expectedTableRowOverhead + rootReserve,
        ),
      );
    const expectedIndexScaled = target === 'candidate_shadow_observations'
      ? selectedRows === 0 || sourceRowsUpper === 0
        ? 0
        : ceilMultiplyDivide(
    ...
    const expectedIndexAdvisory = target === 'candidate_shadow_observations'
      ? expectedIndexScaled == null || !Number.isSafeInteger(expectedIndexRowOverhead)
        ? null
        : roundSharedStagePage(
          expectedIndexScaled + expectedIndexRowOverhead + rootReserve,
        )
      : 0;
  ...
  const sharedStageContractRows = {};
  const sharedStageCapHitTargets = [];
  const sharedStageAdvisoryExceededTargets = [];
  let sharedStageGrantSum = 0;
  let sharedStageActualSum = 0;
  let sharedStageBaselineSum = 0;
  let sharedStageAdvisorySum = 0;
  let sharedStageBorrowedSum = 0;
  ...
  for (const target of expectedSharedStageTargets) {
    ...
    const advisory = Number(raw.advisory_required_bytes);
    const baseline = Number(raw.baseline_required_bytes);
    const allocationWeight = Number(raw.allocation_weight_bytes);
    const grant = Number(raw.granted_cap_bytes);
    const borrowed = Number(raw.borrowed_shared_pool_bytes);
    const advisoryShortfall = Number(raw.advisory_shortfall_bytes);
    const actual = Number(raw.actual_usage_bytes);
    const reportedStageActual = target === 'candidate_shadow_observations'
      ? Number(databaseReports.paper?.temporary_candidate_stage_size_bytes)
      : Number(
        databaseReports.paper?.parallel_paper_stages?.[target]?.stage_size_bytes,
      );
    ...
    const historyHighWater = Number(raw.history_high_water_bytes);
    const historyGrant = Number(raw.history_granted_cap_bytes);
    const historyCapHit = raw.history_cap_hit === true;
    const historyCopyCompleted = raw.history_copy_completed === true;
    const historyState = String(raw.history_state || '');
    const advisoryEvidence = raw.advisory_evidence
      && typeof raw.advisory_evidence === 'object'
      && !Array.isArray(raw.advisory_evidence)
      ? raw.advisory_evidence
      : {};
    const advisoryEvidencePassed = sharedStageAdvisoryEvidencePassed(
      target,
      raw,
      minimum,
      advisory,
    ...
    const selectedTableRows = Number(paperSelections[target]?.rows_copied);
    ...
    let expectedHistoryState = 'none';
    let expectedBaseline = minimum;
    let historyEvidenceValid = Boolean(
      Number.isInteger(historyHighWater)
      && historyHighWater >= 0
      && Number.isInteger(historyGrant)
      && historyGrant >= 0
    );
    if (historyState === 'none') {
      historyEvidenceValid = historyEvidenceValid
        && historyHighWater === 0
        && historyGrant === 0
        && historyCapHit === false
        && historyCopyCompleted === false;
    } else if (historyCapHit) {
      expectedHistoryState = 'cap_hit';
      expectedBaseline = roundSharedStagePage(
        Math.max(minimum, historyHighWater, historyGrant) + 4096,
      );
      historyEvidenceValid = historyEvidenceValid && historyCopyCompleted === false;
      sharedStageCapHitTargets.push(target);
    } else if (historyCopyCompleted) {
      expectedHistoryState = 'completed';
      expectedBaseline = roundSharedStagePage(
        Math.max(minimum, historyHighWater * 1.10),
      );
    } else {
      expectedHistoryState = 'incomplete';
      expectedBaseline = roundSharedStagePage(
        Math.max(minimum, historyHighWater * 1.20),
      );
    }
    const advisoryExceeded = Number.isInteger(actual)
      && Number.isInteger(advisory)
      && actual > advisory;
    const advisoryDelta = Number.isInteger(actual) && Number.isInteger(advisory)
      ? actual - advisory
      : null;
    if (advisoryExceeded) sharedStageAdvisoryExceededTargets.push(target);
    const baseTargetPassed = Boolean(
      raw.target === target
      && raw.stage_filename === sharedStageTargetFilenames[target]
      && Number.isInteger(minimum)
      && minimum === 12288
      && raw.advisory_query_bounded === true
      && raw.physical_upper_bound_claimed === false
      && advisoryEvidencePassed
      && Number.isInteger(advisory)
      && advisory >= minimum
      && advisory % 4096 === 0
      && historyEvidenceValid
      && historyState === expectedHistoryState
      && Number.isInteger(expectedBaseline)
      && Number.isInteger(baseline)
      && baseline === expectedBaseline
      && Number.isInteger(allocationWeight)
      && allocationWeight >= 0
      ...
      && Number.isInteger(advisoryShortfall)
      && advisoryShortfall === Math.max(0, advisory - grant)
      && Number.isInteger(actual)
      && actual > 0
      && Number.isInteger(reportedStageActual)
      && reportedStageActual === actual
      && actual <= grant
      && Number.isInteger(highWater)
      && highWater === actual
      && rowCountBound
      && raw.row_count_bound_to_snapshot === true
      && raw.advisory_exceeded === advisoryExceeded
      && Number.isInteger(advisoryDelta)
      && Number(raw.advisory_delta_bytes) === advisoryDelta
    ...
    if (Number.isInteger(baseline)) sharedStageBaselineSum += baseline;
    if (Number.isInteger(advisory)) sharedStageAdvisorySum += advisory;
    if (Number.isInteger(borrowed)) sharedStageBorrowedSum += borrowed;
    sharedStageContractRows[target] = {
      raw,
      minimum,
      advisory,
      baseline,
    ...
    sharedStageBudgetTargets[target] = {
      target,
      advisory_strategy: typeof raw.advisory_strategy === 'string'
        ? raw.advisory_strategy.slice(0, 80)
        : null,
      advisory_evidence_passed: advisoryEvidencePassed,
      advisory_schema_version:
        typeof advisoryEvidence.advisory_schema_version === 'string'
          ? advisoryEvidence.advisory_schema_version.slice(0, 80)
          : null,
      ...
      table_scaled_physical_advisory_bytes:
        Number.isSafeInteger(
          advisoryEvidence.table_scaled_physical_advisory_bytes,
        )
          ? advisoryEvidence.table_scaled_physical_advisory_bytes
          : null,
      candidate_order_index_advisory_bytes:
        Number.isSafeInteger(
          advisoryEvidence.candidate_order_index_advisory_bytes,
        )
          ? advisoryEvidence.candidate_order_index_advisory_bytes
          : null,
      diagnostic_sample_rows: Number.isSafeInteger(advisoryEvidence.sample_rows)
        ? advisoryEvidence.sample_rows
        : null,
      history_state: historyState || null,
      advisory_required_bytes: Number.isInteger(advisory) ? advisory : null,
      baseline_required_bytes: Number.isInteger(baseline) ? baseline : null,
      allocation_weight_bytes: Number.isInteger(allocationWeight)
        ? allocationWeight
        : null,
      granted_cap_bytes: Number.isInteger(grant) ? grant : null,
      actual_usage_bytes: Number.isInteger(actual) ? actual : null,
      reported_stage_actual_bytes: Number.isInteger(reportedStageActual)
        ? reportedStageActual
        : null,
      ...
      advisory_exceeded: advisoryExceeded,
      advisory_delta_bytes: advisoryDelta,
      advisory_shortfall_bytes: Number.isInteger(advisoryShortfall)
        ? advisoryShortfall
        : null,
  ...
  const sharedStageResidualPool = Number.isInteger(sharedStageTotalCapBytes)
    ? sharedStageTotalCapBytes - sharedStageBaselineSum
    : null;
  const sharedStageExpectedPriority = sharedStageCapHitTargets.length > 0
    ? [...sharedStageCapHitTargets]
    : [...expectedSharedStageTargets];
  const sharedStageExpectedWeights = Object.fromEntries(
    sharedStageExpectedPriority.map((target) => {
      const row = sharedStageContractRows[target] || {};
      return [target, Math.max(4096, row.advisory || 0, row.historyGrant || 0)];
    }),
  );
  const allocateSharedStageResidual = (residualBytes, targets, weights) => {
    if (
      !Number.isSafeInteger(residualBytes)
      || residualBytes < 0
      || residualBytes % 4096 !== 0
    ...
    const normalized = targets.map((target, index) => ({
      target,
      index,
      weight: Math.max(4096, Number(weights[target]) || 0),
    }));
    if (normalized.some(({ weight }) => !Number.isSafeInteger(weight))) return null;
    const totalWeight = normalized.reduce(
      (sum, { weight }) => sum + BigInt(weight),
      0n,
    );
    const totalPages = BigInt(residualBytes / 4096);
    const rows = normalized.map(({ target, index, weight }) => {
      const numerator = totalPages * BigInt(weight);
      return {
        target,
        index,
        pages: numerator / totalWeight,
        remainder: numerator % totalWeight,
      };
    });
    let allocatedPages = rows.reduce((sum, row) => sum + row.pages, 0n);
    let remaining = Number(totalPages - allocatedPages);
    ...
    if (allocatedPages !== totalPages) return null;
    const result = {};
    for (const row of rows) {
      const bytes = row.pages * 4096n;
      if (bytes > BigInt(Number.MAX_SAFE_INTEGER)) return null;
      result[row.target] = Number(bytes);
    }
    return result;
  };
  const sharedStageExpectedAllocations = sharedStageResidualPool != null
    ? allocateSharedStageResidual(
      sharedStageResidualPool,
      sharedStageExpectedPriority,
      sharedStageExpectedWeights,
    )
    : null;
  sharedStageTargetsPassed = sharedStageExpectedAllocations != null;
  for (const target of expectedSharedStageTargets) {
    const row = sharedStageContractRows[target] || {};
    const expectedWeight = sharedStageExpectedWeights[target] || 0;
    const expectedExtra = sharedStageExpectedAllocations?.[target] || 0;
    const allocationPassed = Boolean(
      row.baseTargetPassed === true
      && row.allocationWeight === expectedWeight
      && row.grant === row.baseline + expectedExtra
      && row.borrowed === expectedExtra
    );
    sharedStageTargetsPassed = sharedStageTargetsPassed && allocationPassed;
    if (sharedStageBudgetTargets[target]) {
      sharedStageBudgetTargets[target].passed = allocationPassed;
    }
  }
  ...
  const rawSharedBorrowingPriority = Array.isArray(
    manifestSharedStageBudget.borrowing_priority_targets,
  )
    ? manifestSharedStageBudget.borrowing_priority_targets.map(String)
    : [];
  const rawSharedAdvisoryExceededTargets = Array.isArray(
    manifestSharedStageBudget.targets_exceeding_advisory,
  )
    ? manifestSharedStageBudget.targets_exceeding_advisory.map(String)
    : [];
  ...
  const sharedStageBudgetPassed = Boolean(
    ...
    && manifestSharedStageBudget.schema_version === 'shared_stage_budget.v2'
    ...
    && manifestSharedStageBudget.capacity_sufficient_basis
      === 'minimum_and_verified_history_high_water'
    && manifestSharedStageBudget.all_advisory_queries_bounded === true
    && manifestSharedStageBudget.physical_upper_bound_claimed === false
    && manifestSharedStageBudget.global_hard_cap_enforced === true
    && manifestSharedStageBudget.per_target_max_page_count_enforced === true
    ...
    && rawSharedBorrowingPriority.length === sharedStageExpectedPriority.length
    && rawSharedBorrowingPriority.every(
      (target, index) => target === sharedStageExpectedPriority[index],
    )
    && rawSharedAdvisoryExceededTargets.length
      === sharedStageAdvisoryExceededTargets.length
    && rawSharedAdvisoryExceededTargets.every(
      (target, index) => target === sharedStageAdvisoryExceededTargets[index],
    )
    && Number(manifestSharedStageBudget.advisory_miss_count)
      === sharedStageAdvisoryExceededTargets.length
    && Boolean(manifestSharedStageBudget.history_used)
      === sharedStageHistoryUsedExpected
    && Number(manifestSharedStageBudget.minimum_total_bytes)
      === expectedSharedStageTargets.length * 12288
    && Number(manifestSharedStageBudget.baseline_required_total_bytes)
      === sharedStageBaselineSum
    && Number(manifestSharedStageBudget.advisory_demand_total_bytes)
      === sharedStageAdvisorySum
    && Number(manifestSharedStageBudget.allocation_weight_total_bytes)
      === Object.values(sharedStageExpectedWeights).reduce(
        (sum, value) => sum + value,
        0,
      )
    && Number(manifestSharedStageBudget.residual_pool_bytes)
      === sharedStageResidualPool
    && sharedStageBorrowedSum === sharedStageResidualPool
  ...
  for (const target of [
    ...
    const evidenceNumeric = (field) => (
      typeof advisoryEvidence[field] === 'number'
      && Number.isFinite(advisoryEvidence[field])
        ? advisoryEvidence[field]
        : null
    );
    const publicAdvisory = numeric('advisory_required_bytes');
    const publicMinimum = numeric('minimum_cap_bytes');
    publicSharedStageBudgetTargets[target] = {
      advisory_required_bytes: publicAdvisory,
      baseline_required_bytes: numeric('baseline_required_bytes'),
      allocation_weight_bytes: numeric('allocation_weight_bytes'),
      ...
      advisory_shortfall_bytes: numeric('advisory_shortfall_bytes'),
      advisory_delta_bytes: numeric('advisory_delta_bytes'),
      ...
      advisory_schema_version:
        typeof advisoryEvidence.advisory_schema_version === 'string'
          ? advisoryEvidence.advisory_schema_version.slice(0, 80)
          : null,
      advisory_evidence_passed: publicAdvisory != null && publicMinimum != null
        ? sharedStageAdvisoryEvidencePassed(
          target,
          raw,
          publicMinimum,
          publicAdvisory,
      ...
      table_scaled_physical_advisory_bytes: evidenceNumeric(
        'table_scaled_physical_advisory_bytes',
      ),
      table_advisory_bytes: evidenceNumeric('table_advisory_bytes'),
      candidate_order_index_advisory_bytes: evidenceNumeric(
        'candidate_order_index_advisory_bytes',
      ),
      diagnostic_sample_rows: evidenceNumeric('sample_rows'),
      pinned_read_view_role:
        typeof advisoryEvidence.pinned_read_view_role === 'string'
          ? advisoryEvidence.pinned_read_view_role.slice(0, 80)
          : null,
      estimate_started_after_pin:
        advisoryEvidence.estimate_started_after_pin === true,
      estimate_completed_before_copy:
        advisoryEvidence.estimate_completed_before_copy === true,
      history_state: typeof raw.history_state === 'string'
        ? raw.history_state.slice(0, 40)
        : null,
      advisory_exceeded: raw.advisory_exceeded === true,
      copy_completed: raw.copy_completed === true,
      cap_hit: raw.cap_hit === true,
      within_grant: raw.within_grant === true,
    };
  ...
  const publicSharedStageBudget = {
    available: publicSharedBudgetSource.schema_version === 'shared_stage_budget.v2',
    ...
    capacity_sufficient_basis:
      publicSharedBudgetSource.capacity_sufficient_basis || null,
    global_hard_cap_enforced:
      publicSharedBudgetSource.global_hard_cap_enforced === true,
    per_target_max_page_count_enforced:
      publicSharedBudgetSource.per_target_max_page_count_enforced === true,
    physical_upper_bound_claimed:
      publicSharedBudgetSource.physical_upper_bound_claimed === true,
    all_advisory_queries_bounded:
      publicSharedBudgetSource.all_advisory_queries_bounded === true,
    ...
    all_advisory_estimates_pinned_read_view_bound:
      publicSharedBudgetSource
        .all_advisory_estimates_pinned_read_view_bound === true,
    advisory_demand_total_bytes: Number.isFinite(
      Number(publicSharedBudgetSource.advisory_demand_total_bytes),
    )
      ? Number(publicSharedBudgetSource.advisory_demand_total_bytes)
      : null,
    advisory_miss_count: Number.isSafeInteger(
      publicSharedBudgetSource.advisory_miss_count,
    )
      ? publicSharedBudgetSource.advisory_miss_count
      : null,
    targets_exceeding_advisory: Array.isArray(
      publicSharedBudgetSource.targets_exceeding_advisory,
    )
      ? publicSharedBudgetSource.targets_exceeding_advisory
        .map(String)
        .filter((target) => Object.hasOwn(sharedStageTargetFilenames, target))
  ...
  const sharedStageAdvisoryReadViewBindingsPassed = Boolean(
    manifestPayload?.shared_stage_estimates_bound_to_copy_read_views === true
    ...
    && Object.entries(expectedSharedStagePinnedRoles).every(([target, role]) => {
      const evidence = rawSharedTargets[target]?.advisory_evidence;
  ...
  publicSharedStageBudget.pinned_read_view_bindings_passed = (
    sharedStageAdvisoryReadViewBindingsPassed
  );
  publicSharedStageBudget.contract_passed = Boolean(
    sharedStageBudgetPassed && sharedStageAdvisoryReadViewBindingsPassed
  );
...
const server = http.createServer(async (req, res) => {
  ...
  if (url.pathname === '/' || url.pathname === '/health' || url.pathname === '/ping') {
    ...
    const aliases = {
      ...
      latest_handoff_json: 'handoff_json',
      codex_handoff_json: 'handoff_json',
      ...
      capture_60_gap: 'capture_60_gap_report',
      capture_gap_report: 'capture_60_gap_report',
      stage_metrics: 'capture_stage_metrics',
      context_coverage_audit: 'context_coverage',
      context_dimension_eligibility_report: 'context_dimension_eligibility',
      dimension_eligibility: 'context_dimension_eligibility',
      ...
      pending_to_final: 'pending_to_final_entry_audit',
      pending_to_final_audit: 'pending_to_final_entry_audit',
      final_entry_readiness: 'final_entry_readiness_audit',
      candidate_effectiveness_report: 'candidate_effectiveness',
      candidate_improvement_opportunities: 'candidate_improvement',
      strategy_memory_capture: 'strategy_memory_capture_validation',
      strategy_memory_capture_report: 'strategy_memory_capture_validation',
      shadow_candidate_queue: 'shadow_candidate_improvement_queue',
      capture_cross_validity_report: 'capture_cross_validity',
      clean_dimension_capture_cross: 'clean_dimension_2d_capture_cross',
      clean_dimension_2d_capture_cross_report: 'clean_dimension_2d_capture_cross',
      quality_timing_reason_cross_report: 'quality_timing_reason_cross',
      strategy_memory_reason_cross_report: 'strategy_memory_reason_cross',
      pnl_cross: 'pnl',
      ...
      markov_candidate_volume: 'markov_candidate_volume',
      markov_candidate_quote: 'markov_candidate_quote',
      candidate_volume_markov: 'markov_candidate_volume',
      candidate_quote_markov: 'markov_candidate_quote',
      ...
      oos_readiness_summary_report: 'oos_readiness_summary',
      pass_allow_oos_freeze_registry: 'pass_allow_60_oos_freeze_registry',
      pass_allow_60_oos_freeze: 'pass_allow_60_oos_freeze_registry',
      pass_allow_post_freeze_oos_validation: 'pass_allow_60_post_freeze_oos_validation',
      pass_allow_60_oos_validation: 'pass_allow_60_post_freeze_oos_validation',
      capture_cross_oos_freeze: 'capture_cross_oos_freeze_registry',
      capture_cross_freeze_registry: 'capture_cross_oos_freeze_registry',
      capture_cross_post_freeze_oos: 'capture_cross_post_freeze_oos_validation',
      capture_cross_oos_validation: 'capture_cross_post_freeze_oos_validation',
      p7_exit_policy_oos: 'p7_exit_policy_oos_validation',
      p7_exit_policy_oos_validation: 'p7_exit_policy_oos_validation',
      p7_oos_validation: 'p7_exit_policy_oos_validation',
      exit_policy_oos_validation: 'p7_exit_policy_oos_validation',
      phase3: 'phase3_goal_loop',
      phase3_goal: 'phase3_goal_loop',
      phase3_goal_loop: 'phase3_goal_loop',
      phase3_summary: 'phase3_goal_loop_summary',
      phase3_goal_loop_summary: 'phase3_goal_loop_summary',
      p7_paper_proposal: 'p7_paper_proposal_checkpoint',
      paper_exit_policy_proposal: 'p7_paper_proposal_checkpoint',
      p7_paper_proposal_checkpoint: 'p7_paper_proposal_checkpoint',
      p7_paper_proposal_md: 'p7_paper_proposal_checkpoint_md',
      paper_exit_policy_proposal_md: 'p7_paper_proposal_checkpoint_md',
      phase3_wide_net_paper_contract: 'phase3_wide_net_paper_contract',
      wide_net_paper_contract: 'phase3_wide_net_paper_contract',
      phase3_wide_net_paper_experiment: 'phase3_wide_net_paper_experiment_summary',
      phase3_wide_net_paper_experiment_summary: 'phase3_wide_net_paper_experiment_summary',
      wide_net_paper_experiment: 'phase3_wide_net_paper_experiment_summary',
      p7_human_review: 'phase3_p7_human_review_packet',
      p7_human_review_packet: 'phase3_p7_human_review_packet',
      p7_human_review_md: 'phase3_p7_human_review_packet_md',
      phase3_p7_human_review_packet: 'phase3_p7_human_review_packet',
      phase3_p7_human_review_packet_md: 'phase3_p7_human_review_packet_md',
      phase3_24h_path_observer: 'phase3_24h_path_observer_summary',
      phase3_24h_path_observer_summary: 'phase3_24h_path_observer_summary',
      phase3_path_horizon: 'phase3_path_horizon_audit',
      phase3_path_horizon_audit: 'phase3_path_horizon_audit',
      p9_metric_ledger: 'p9_metric_predictiveness_ledger',
      p9_metric_predictiveness_ledger: 'p9_metric_predictiveness_ledger',
      influence_kol: 'influence_kol_shadow_source_plan',
      influence_kol_shadow: 'influence_kol_shadow_source_plan',
      influence_kol_shadow_source_plan: 'influence_kol_shadow_source_plan',
      influence_kol_shadow_features: 'influence_kol_shadow_features',
      ...
      pending_stale_before_final_watch: 'pending_stale_before_final_watch_validation',
      pending_stale_before_final_watch_validation: 'pending_stale_before_final_watch_validation',
      pending_stale_watch_validation: 'pending_stale_before_final_watch_validation',
      pump_fun_shadow_summary: 'pump_fun_shadow_observer_summary',
      pump_fun_observer_summary: 'pump_fun_shadow_observer_summary',
      pump_fun_shadow_source_comparison: 'pump_fun_shadow_source_comparison',
      pump_fun_source_comparison: 'pump_fun_shadow_source_comparison',
      pump_fun_p8_comparison: 'pump_fun_shadow_source_comparison',
      pump_fun_shadow_source_comparison_30d: 'pump_fun_shadow_source_comparison_30d',
      pump_fun_p8_comparison_30d: 'pump_fun_shadow_source_comparison_30d',
      pump_fun_shadow_worker_status: 'pump_fun_shadow_worker_status',
      pump_fun_worker_status: 'pump_fun_shadow_worker_status',
    ...
    const payload = {
      ...
      downloads: {
        ...
        agent_capture_60_gap_report_json: `${origin}/api/data/download/agent-capture-discovery?token=${tokenHint}&artifact=capture_60_gap_report`,
        agent_capture_stage_metrics_json: `${origin}/api/data/download/agent-capture-discovery?token=${tokenHint}&artifact=capture_stage_metrics`,
        agent_context_coverage_json: `${origin}/api/data/download/agent-capture-discovery?token=${tokenHint}&artifact=context_coverage`,
        agent_context_dimension_eligibility_json: `${origin}/api/data/download/agent-capture-discovery?token=${tokenHint}&artifact=context_dimension_eligibility`,
        ...
        agent_pending_to_final_entry_audit_json: `${origin}/api/data/download/agent-capture-discovery?token=${tokenHint}&artifact=pending_to_final_entry_audit`,
        agent_final_entry_readiness_audit_json: `${origin}/api/data/download/agent-capture-discovery?token=${tokenHint}&artifact=final_entry_readiness_audit`,
        ...
        agent_strategy_memory_capture_validation_json: `${origin}/api/data/download/agent-capture-discovery?token=${tokenHint}&artifact=strategy_memory_capture_validation`,
        agent_shadow_candidate_improvement_queue_json: `${origin}/api/data/download/agent-capture-discovery?token=${tokenHint}&artifact=shadow_candidate_improvement_queue`,
        ...
        agent_clean_dimension_2d_capture_cross_json: `${origin}/api/data/download/agent-capture-discovery?token=${tokenHint}&artifact=clean_dimension_2d_capture_cross`,
        agent_quality_timing_reason_cross_json: `${origin}/api/data/download/agent-capture-discovery?token=${tokenHint}&artifact=quality_timing_reason_cross`,
        agent_strategy_memory_reason_cross_json: `${origin}/api/data/download/agent-capture-discovery?token=${tokenHint}&artifact=strategy_memory_reason_cross`,
        ...
        agent_markov_candidate_volume_json: `${origin}/api/data/download/agent-capture-discovery?token=${tokenHint}&artifact=markov_candidate_volume`,
        agent_markov_candidate_quote_json: `${origin}/api/data/download/agent-capture-discovery?token=${tokenHint}&artifact=markov_candidate_quote`,
        ...
        agent_oos_readiness_summary_json: `${origin}/api/data/download/agent-capture-discovery?token=${tokenHint}&artifact=oos_readiness_summary`,
        agent_pass_allow_60_oos_freeze_registry_json: `${origin}/api/data/download/agent-capture-discovery?token=${tokenHint}&artifact=pass_allow_60_oos_freeze_registry`,
        agent_pass_allow_60_post_freeze_oos_validation_json: `${origin}/api/data/download/agent-capture-discovery?token=${tokenHint}&artifact=pass_allow_60_post_freeze_oos_validation`,
        agent_capture_cross_oos_freeze_registry_json: `${origin}/api/data/download/agent-capture-discovery?token=${tokenHint}&artifact=capture_cross_oos_freeze_registry`,
        agent_capture_cross_post_freeze_oos_validation_json: `${origin}/api/data/download/agent-capture-discovery?token=${tokenHint}&artifact=capture_cross_post_freeze_oos_validation`,
        agent_p7_exit_policy_oos_validation_json: `${origin}/api/data/download/agent-capture-discovery?token=${tokenHint}&artifact=p7_exit_policy_oos_validation`,
        agent_phase3_goal_loop_json: `${origin}/api/data/download/agent-capture-discovery?token=${tokenHint}&artifact=phase3_goal_loop`,
        agent_phase3_goal_loop_md: `${origin}/api/data/download/agent-capture-discovery?token=${tokenHint}&artifact=phase3_goal_loop_summary`,
        agent_p7_paper_proposal_checkpoint_json: `${origin}/api/data/download/agent-capture-discovery?token=${tokenHint}&artifact=p7_paper_proposal_checkpoint`,
        agent_p7_paper_proposal_checkpoint_md: `${origin}/api/data/download/agent-capture-discovery?token=${tokenHint}&artifact=p7_paper_proposal_md`,
        agent_phase3_wide_net_paper_contract_json: `${origin}/api/data/download/agent-capture-discovery?token=${tokenHint}&artifact=phase3_wide_net_paper_contract`,
        agent_phase3_wide_net_paper_experiment_json: `${origin}/api/data/download/agent-capture-discovery?token=${tokenHint}&artifact=phase3_wide_net_paper_experiment`,
        agent_phase3_p7_human_review_packet_json: `${origin}/api/data/download/agent-capture-discovery?token=${tokenHint}&artifact=phase3_p7_human_review_packet`,
        agent_phase3_p7_human_review_packet_md: `${origin}/api/data/download/agent-capture-discovery?token=${tokenHint}&artifact=phase3_p7_human_review_packet_md`,
        agent_phase3_24h_path_observer_json: `${origin}/api/data/download/agent-capture-discovery?token=${tokenHint}&artifact=phase3_24h_path_observer_summary`,
        agent_phase3_path_horizon_audit_json: `${origin}/api/data/download/agent-capture-discovery?token=${tokenHint}&artifact=phase3_path_horizon_audit`,
        agent_p9_metric_predictiveness_ledger_json: `${origin}/api/data/download/agent-capture-discovery?token=${tokenHint}&artifact=p9_metric_predictiveness_ledger`,
        agent_influence_kol_shadow_source_plan_json: `${origin}/api/data/download/agent-capture-discovery?token=${tokenHint}&artifact=influence_kol_shadow_source_plan`,
        agent_influence_kol_shadow_features_json: `${origin}/api/data/download/agent-capture-discovery?token=${tokenHint}&artifact=influence_kol_shadow_features`,
        ...
        agent_pending_stale_before_final_watch_validation_json: `${origin}/api/data/download/agent-capture-discovery?token=${tokenHint}&artifact=pending_stale_before_final_watch_validation`,
        agent_pump_fun_shadow_observer_summary_json: `${origin}/api/data/download/agent-capture-discovery?token=${tokenHint}&artifact=pump_fun_shadow_observer_summary`,
        agent_pump_fun_shadow_source_comparison_json: `${origin}/api/data/download/agent-capture-discovery?token=${tokenHint}&artifact=pump_fun_shadow_source_comparison`,
        agent_pump_fun_shadow_source_comparison_30d_json: `${origin}/api/data/download/agent-capture-discovery?token=${tokenHint}&artifact=pump_fun_shadow_source_comparison_30d`,
        agent_pump_fun_shadow_worker_status_json: `${origin}/api/data/download/agent-capture-discovery?token=${tokenHint}&artifact=pump_fun_shadow_worker_status`,
