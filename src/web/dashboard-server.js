...
const server = http.createServer(async (req, res) => {
  ...
  if (url.pathname === '/' || url.pathname === '/health' || url.pathname === '/ping') {
    ...
    return;
  } else if (url.pathname === '/api/paper/candidate-shadow-runtime-cross-summary') {
    if (!checkAuth(req, url, res)) return;
    const summaryPath = process.env.CANDIDATE_SHADOW_RUNTIME_CROSS_SUMMARY_PATH || '/app/data/candidate_shadow_runtime_cross_summary.json';
    try {
      if (!fs.existsSync(summaryPath)) {
        res.writeHead(404, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({
          error: 'candidate_shadow_runtime_cross_summary_not_ready',
          materialized: true,
          path: summaryPath,
          note: 'Generated by scripts/materialize_candidate_shadow_runtime_cross.py; dashboard does not run this DB join inline.',
        }, null, 2));
        return;
      }
      const snapshot = JSON.parse(fs.readFileSync(summaryPath, 'utf8'));
      const dim = url.searchParams.get('dimension');
      if (dim) {
        const rows = snapshot?.dimensions?.[dim];
        if (!Array.isArray(rows)) {
          res.writeHead(400, { 'Content-Type': 'application/json' });
          res.end(JSON.stringify({
            error: 'unsupported_dimension',
            dimension: dim,
            supported_dimensions: Object.keys(snapshot?.dimensions || {}),
          }, null, 2));
          return;
        }
        res.writeHead(200, { 'Content-Type': 'application/json; charset=utf-8' });
        res.end(JSON.stringify({
          schema_version: 'candidate_shadow_runtime_cross_summary.dimension.v1',
          materialized: true,
          generated_at: snapshot.generated_at,
          window_hours: snapshot.window_hours,
          since_ts: snapshot.since_ts,
          since_iso: snapshot.since_iso,
          coverage: snapshot.coverage,
          min_closed: snapshot.min_closed,
          dimension: dim,
          rows,
        }, null, 2));
        return;
      ...
      res.writeHead(500, { 'Content-Type': 'application/json' });
      res.end(JSON.stringify({ error: e.message, path: summaryPath }));
    }
    return;
