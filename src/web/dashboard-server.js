...
function summarizeCandidateShadowTrades(rows, observationRows = []) {
  ...
  for (const row of observationRows) {
    ...
    if (!byCandidate.has(candidateId)) {
      byCandidate.set(candidateId, {
        ...
        unique_observation_tokens: new Set(),
        virtual_rows: 0,
        closed_n: 0,
        open_n: 0,
        waiting_n: 0,
    ...
    item.observation_rows += Number(row.observation_rows || 0);
    item.matched_observations += Number(row.matched_observations || 0);
    if (row.unique_tokens) item.unique_observation_tokens_count = Number(row.unique_tokens || 0);
  }
  for (const row of rows) {
    ...
    const item = byCandidate.get(candidateId);
    item.family ||= row.family || null;
    item.virtual_rows += 1;
    if (row.token_ca) item.unique_tokens.add(row.token_ca);
    if (row.status === 'VIRTUAL_CLOSED') {
      item.closed_n += 1;
      const pnl = nullableFiniteNumber(row.net_pnl_pct);
      if (pnl != null) {
        item.pnls.push(pnl);
        if (pnl > 0) item.wins += 1;
      }
      const entryTs = nullableFiniteNumber(row.entry_ts);
      const exitTs = nullableFiniteNumber(row.exit_ts);
      if (entryTs != null && exitTs != null && exitTs >= entryTs) {
        item.hold_minutes.push((exitTs - entryTs) / 60);
      }
    } else if (row.status === 'VIRTUAL_OPEN') {
      item.open_n += 1;
    } else {
      item.waiting_n += 1;
    }
  }
  return [...byCandidate.values()].map((item) => {
    const pnls = item.pnls.slice().sort((a, b) => a - b);
    const total = pnls.reduce((sum, pnl) => sum + pnl, 0);
    const wins = pnls.filter((pnl) => pnl > 0);
    const losses = pnls.filter((pnl) => pnl < 0);
    const sumWins = wins.reduce((sum, pnl) => sum + pnl, 0);
    const sumLosses = losses.reduce((sum, pnl) => sum + pnl, 0);
    const best = pnls.length ? Math.max(...pnls) : null;
    return {
      candidate_id: item.candidate_id,
      family: item.family,
      observation_rows: item.observation_rows,
      matched_observations: item.matched_observations,
      match_rate_pct: item.observation_rows ? roundNumber(item.matched_observations / item.observation_rows * 100, 2) : null,
      unique_observation_tokens: item.unique_observation_tokens_count ?? item.unique_observation_tokens.size,
      virtual_rows: item.virtual_rows,
      closed_n: item.closed_n,
      open_n: item.open_n,
      waiting_n: item.waiting_n,
      waiting_rate_pct: item.virtual_rows ? roundNumber(item.waiting_n / item.virtual_rows * 100, 2) : null,
      unique_tokens: item.unique_tokens.size,
      wins: item.wins,
      win_rate_pct: item.closed_n ? roundNumber(item.wins / item.closed_n * 100, 2) : null,
      median_net_pnl_pct: pnls.length ? roundNumber(percentileNumber(pnls, 50), 4) : null,
      avg_net_pnl_pct: pnls.length ? roundNumber(total / pnls.length, 4) : null,
      total_net_pnl_pct: pnls.length ? roundNumber(total, 4) : null,
      total_minus_best_net_pnl_pct: best == null ? null : roundNumber(total - best, 4),
      best_net_pnl_pct: best == null ? null : roundNumber(best, 4),
      worst_net_pnl_pct: pnls.length ? roundNumber(Math.min(...pnls), 4) : null,
      profit_factor: sumLosses < 0 ? roundNumber(sumWins / Math.abs(sumLosses), 4) : (sumWins > 0 ? null : 0),
      max_drawdown_pct: pnls.length ? roundNumber(maxDrawdownPct(item.pnls), 4) : null,
      max_contribution_pct: best != null && sumWins > 0 ? roundNumber(best / sumWins * 100, 2) : null,
      avg_hold_minutes: item.hold_minutes.length
        ? roundNumber(item.hold_minutes.reduce((sum, value) => sum + value, 0) / item.hold_minutes.length, 2)
        : null,
    };
  }).sort((a, b) => (
    (b.closed_n - a.closed_n)
    || ((b.median_net_pnl_pct ?? -Infinity) - (a.median_net_pnl_pct ?? -Infinity))
...
const server = http.createServer(async (req, res) => {
  ...
  if (url.pathname === '/' || url.pathname === '/health' || url.pathname === '/ping') {
    ...
    try {
      ...
      const tableNames = new Set(paperDb.prepare("SELECT name FROM sqlite_master WHERE type='table'").all().map((row) => row.name));
      if (!tableNames.has('candidate_shadow_observations') || !tableNames.has('candidate_shadow_virtual_trades')) {
        res.writeHead(404, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({
          error: 'candidate_shadow_tables_not_found',
          has_observations: tableNames.has('candidate_shadow_observations'),
          has_virtual_trades: tableNames.has('candidate_shadow_virtual_trades'),
        }, null, 2));
        return;
      }
      ...
      `).all({ since: sinceTs });
      const virtualRows = paperDb.prepare(`
        SELECT candidate_id, family, status, token_ca, entry_ts, exit_ts, net_pnl_pct, observed_at
        FROM candidate_shadow_virtual_trades
        WHERE observed_at >= @since
        ORDER BY observed_at DESC, id DESC
        LIMIT @limit
      `).all({ since: sinceTs, limit });
      const totalVirtualRows = paperDb.prepare(`
        SELECT COUNT(*) AS n
        FROM candidate_shadow_virtual_trades
        WHERE observed_at >= @since
      `).get({ since: sinceTs })?.n || 0;
      const candidates = summarizeCandidateShadowTrades(virtualRows, observationRows);
      res.writeHead(200, { 'Content-Type': 'application/json; charset=utf-8' });
      res.end(JSON.stringify({
        ...
        virtual: {
          rows_scanned: virtualRows.length,
          total_rows_in_window: totalVirtualRows,
          truncated: Number(totalVirtualRows) > virtualRows.length,
        },
        candidates,
        query_ms: Date.now() - startedAt,
      }, null, 2));
