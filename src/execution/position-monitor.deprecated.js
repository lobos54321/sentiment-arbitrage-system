...
export class PositionMonitor {
  constructor(config, db) {
    ...
    // Exit thresholds - v5.0 Master Strategy
    this.thresholds = {
      ...
      // Standard SOP (Updated)
      SMART_TIME_STOP_30_MIN: 30, // 30 min check
      SMART_TIME_STOP_60_MIN: 60, // 60 min check

      PROFIT_TARGET_1: 0.30, // +30% → sell 50%
      PROFIT_TARGET_2: 0.50, // +50% → sell remaining 50%
      STOP_LOSS: -0.30 // -30% → exit all (Tightened from -50% to -30%)
    };

    console.log('📊 Position Monitor v5.0 initialized');
  ...
  evaluateExitTriggers(position, snapshot, tgData, pnl) {
    ...
    if (!shouldExit) {
      ...
      // 3.3 Smart Time Stops (v5.0)
      const holdTimeMinutes = this.getHoldTimeMinutes(position.entry_time);

      // Check 1: 30 minutes check
      // 如果持有30分钟还在亏损 > 10% (且不是止损)，说明走势很弱 → 减半仓位或止损
      if (holdTimeMinutes > this.thresholds.SMART_TIME_STOP_30_MIN && holdTimeMinutes < this.thresholds.SMART_TIME_STOP_60_MIN) {
        if (pnl.pnl_percent < -10) {
          shouldExit = true;
          exitType = 'time_stop_weak';
          sellPercent = 100;
          exitReason.push(`Weak after 30m: ${pnl.pnl_percent.toFixed(1)}%`);
        }
      }

      // Check 2: 60 minutes check
      // 60分钟后如果不涨不跌 (+10% ~ -10%) → 换车，清仓
      else if (holdTimeMinutes > this.thresholds.SMART_TIME_STOP_60_MIN) {
        // 如果盈利超过 20%，则忽略时间止损，让利润奔跑
        const isProfitable = pnl.pnl_percent > 20;

        if (!isProfitable) {
          shouldExit = true;
          exitType = 'time_stop_stagnant';
          sellPercent = 100;
          exitReason.push(`Stagnant after 60m: ${pnl.pnl_percent.toFixed(1)}%`);
        }
      }
    }
