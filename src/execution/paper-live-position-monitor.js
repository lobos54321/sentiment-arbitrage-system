...
function pctToDecimal(value, fallback = 0) {
...
function buildPaperQuoteFailureExecution(position = {}, sellAmount = 0, failureReason = 'sell_execution_failed', routeAvailable = null, quoteTsSec = 0) {
  const tokenDecimals = Math.max(0, Math.trunc(toNumber(position.tokenDecimals, 0)));
  const divisor = tokenDecimals > 0 ? Math.pow(10, tokenDecimals) : 1;
  return {
    mode: 'paper',
    ...
    inputAmount: sellAmount > 0 ? sellAmount / divisor : null,
    inputAmountRaw: sellAmount > 0 ? String(Math.trunc(sellAmount)) : null,
    inputMint: position.tokenCA || null,
    outputMint: SOL_MINT,
    inputDecimals: tokenDecimals,
    outputDecimals: 9,
    tokenCA: position.tokenCA || null,
    error: null,
...
export function evaluatePaperLiveManagedExit({ position = {}, mark = {} } = {}) {
  ...
  const entryTs = Number(position.entryTs || 0);
  const peakPnl = Number(position.peakPnl || 0);
  const trailingActive = Boolean(position.trailingActive);
  const exitRules = position.exitRules || {};

  if (!Number.isFinite(currentPrice) || currentPrice <= 0 || !Number.isFinite(entryPrice) || entryPrice <= 0 || !Number.isFinite(quoteTsSec) || quoteTsSec <= 0 || !Number.isFinite(entryTs) || entryTs <= 0) {
    return {
      ok: false,
      shouldExit: false,
      exitReason: null,
      ...
      updatedState: {
        peakPnl,
        trailingActive,
        barsHeld: Number(position.barsHeld || 0),
        lastMarkTs: Number(position.lastMarkTs || position.lastBarTs || entryTs),
      },
      holdTimeMin: null,
    };
  }

  const stopLoss = -pctToDecimal(exitRules.stopLossPct, 3);
  const trailStart = pctToDecimal(exitRules.trailStartPct, 2);
  const trailFactor = Number.isFinite(Number(exitRules.trailFactor)) ? Number(exitRules.trailFactor) : 0.9;
  const timeoutMin = Number.isFinite(Number(exitRules.timeoutMinutes)) ? Number(exitRules.timeoutMinutes) : 120;

  const triggerPnl = (currentPrice - entryPrice) / entryPrice;
  const nextPeakPnl = Math.max(peakPnl, triggerPnl);
  const nextTrailingActive = trailingActive || nextPeakPnl >= trailStart;
  const justActivatedTrailing = !trailingActive && nextTrailingActive;
  const holdTimeMin = (quoteTsSec - entryTs) / 60;
  const barsHeld = Math.max(Number(position.barsHeld || 0), Math.floor(Math.max(0, quoteTsSec - entryTs) / 60) + 1);

  let shouldExit = false;
  let exitReason = null;

  if (!nextTrailingActive && triggerPnl <= stopLoss) {
    shouldExit = true;
    exitReason = 'sl';
  } else if (nextTrailingActive) {
    const trailLevel = nextPeakPnl * trailFactor;
    if (triggerPnl <= trailLevel) {
      shouldExit = true;
      exitReason = 'trail';
    }
  }
  ...
  return {
    ...
    updatedState: {
      peakPnl: nextPeakPnl,
      trailingActive: nextTrailingActive,
      barsHeld,
      lastMarkTs: quoteTsSec,
    },
    holdTimeMin,
  };
}

export async function evaluatePaperLiveManagedPosition({ position = {}, mark = {}, executor = null } = {}) {
  const tokenCA = position.tokenCA || null;
  const tokenAmountRaw = Number(position.tokenAmountRaw || 0);
  const tokenDecimals = Number(position.tokenDecimals || 0);
  const solPriceUsd = Number(mark.solPriceUsd || 0);

  let preQuote = null;
  let currentPrice = Number(mark.currentPrice || 0);
  let quoteTsSec = Number(mark.quoteTsSec || 0);
  let markSource = 'fallback';
  let routeAvailable = null;
  let quoteFailureReason = null;
  ...
  if (executor && tokenCA && tokenAmountRaw > 0) {
    ...
    if (managedMark?.ok) {
      ...
      const markSol = Number(managedMark.currentPriceSol || 0);
      currentPrice = markUsd > 0
        ? markUsd
        : (markSol > 0 && solPriceUsd > 0 ? markSol * solPriceUsd : (markSol || currentPrice || 0));
      quoteTsSec = Number(managedMark.quoteTsSec || quoteTsSec || 0);
      markSource = managedMark.source || 'quote';
      preQuote = managedMark.routeAvailable === true
        ? await executor.simulateSell(tokenCA, tokenAmountRaw, {
            stage: position.strategyStage,
            strategyId: position.strategyId,
            lifecycleId: position.lifecycleId,
            inputAmount: tokenDecimals >= 0 ? (tokenAmountRaw / Math.pow(10, tokenDecimals || 0)) : null,
            applyPaperPenalty: true,
          })
        : null;

      if (preQuote) {
        routeAvailable = preQuote.routeAvailable ?? routeAvailable;
        quoteFailureReason = preQuote.failureReason || quoteFailureReason;
        quotedOutAmount = preQuote.quotedOutAmount ?? quotedOutAmount;
      }
    } else {
      markSource = `fallback:${managedMark?.quoteFailureReason || 'unavailable'}`;
      quoteTsSec = Number(managedMark?.quoteTsSec || quoteTsSec || 0);
    }

    if (routeAvailable === false && (quoteFailureReason === 'token_not_tradable' || quoteFailureReason === 'no_route')) {
      terminalQuoteFailureReason = quoteFailureReason;
    }
  }
  ...
  if (!execution) {
    execution = await executor.simulateSell(
      position.tokenCA,
      sellAmount,
  ...
  if (!execution?.success) {
    const effectiveFailureReason = execution?.failureReason || terminalQuoteFailureReason || quoteFailureReason || 'sell_execution_failed';
    const failureExecution = execution || buildPaperQuoteFailureExecution(
      position,
      sellAmount,
    ...
    return {
      ...
      currentPrice,
      quoteTsSec,
      routeAvailable: failureExecution?.routeAvailable ?? routeAvailable,
      quoteFailureReason: effectiveFailureReason,
      quotedOutAmount: failureExecution?.quotedOutAmount ?? quotedOutAmount,
    };
  }
