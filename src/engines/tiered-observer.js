...
import snapshotRecorder from '../database/signal-snapshot-recorder.js';  // v9.4: AI决策追踪

// 三级池策略配置 v6.7 (势利眼 + 拾荒者)
const TIERED_POOL = {
    // 1. 池子容量 (总共 50 个)
    CAPACITY: {
        GOLD: 10,     // 核心区: 最多10个 (原5个)
        SILVER: 40,   // 缓冲区: 最多40个 (原15个)
        BRONZE: 100   // 海选区: 最多100个 (原30个)
    },
    ...
    // 3. 动态晋级条件
    PROMOTION: {
        TO_SILVER: { smDelta: 2, priceDelta: 15 },  // 铜 -> 银
        TO_GOLD: { smDelta: 3, priceDelta: 25 }     // 银 -> 金
    },

    // 4. 最小观察时间
    MIN_OBSERVE_TIME: {
        GOLD: 5 * 60 * 1000,     // 金池至少观察 5 分钟
        SILVER: 8 * 60 * 1000,   // 银池至少观察 8 分钟
        BRONZE: 10 * 60 * 1000   // 铜池至少观察 10 分钟
    },

    // 5. 降级/淘汰条件 (v6.2 增强)
    EVICTION: {
        DROP_GOLD: { smDelta: -2, priceDelta: -15 },   // 金 -> 银: SM-2 或 跌15%
        DROP_SILVER: { smDelta: -1, priceDelta: -10 }, // 银 -> 铜: SM-1 且 跌10%
        MAX_IDLE_TIME: 15 * 60 * 1000                  // 铜池最大空闲 15 分钟
    },
    ...
    GRADUATION_REQ: {
        // 金池毕业要求 (只有金池可以毕业买入)
        GOLD: {
            MIN_SM_DELTA: 0,            // SM 不能流出 (原-1)
            MAX_PRICE_DROP: -0.05,      // 价格不能跌超 5% (原-10%)
...
class TieredObserver extends EventEmitter {
    constructor(config = {}) {
        ...
        console.log(`🏟️  [TieredObserver] 三级观察室初始化完成`);
        console.log(`   金池容量: ${this.params.CAPACITY.GOLD} (观察 ${this.params.MIN_OBSERVE_TIME.GOLD / 60000} 分钟)`);
        console.log(`   银池容量: ${this.params.CAPACITY.SILVER} (观察 ${this.params.MIN_OBSERVE_TIME.SILVER / 60000} 分钟)`);
        console.log(`   铜池容量: ${this.params.CAPACITY.BRONZE} (观察 ${this.params.MIN_OBSERVE_TIME.BRONZE / 60000} 分钟)`);
    ...
    async addToken(signal, analysis) {
        ...
        if (this.tokens.has(tokenCA)) {
            ...
            if (analysis.score > existing.score) {
        ...
        const score = typeof rawScore === 'number' ? rawScore : (rawScore?.total || 50);
        const factors = analysis?.dynamicFactors || {};

        // 1. 定级
        let tier = 'BRONZE';
        if (score >= this.params.ENTRY_THRESHOLD.GOLD || factors.tag === 'GOLDEN') {
            tier = 'GOLD';
        } else if (score >= this.params.ENTRY_THRESHOLD.SILVER) {
            tier = 'SILVER';
        }

        // ═══════════════════════════════════════════════════════════════
        // v9.0 STABLE 高分快速通道 (Gold Dog Protection)
        // 79.3% 被误杀金狗是 STABLE 信号，86.2% 基础分 ≥55
        // 条件: STABLE + baseScore≥58 + SM≥2 → 直接进入金池
        // ═══════════════════════════════════════════════════════════════
        const signalType = factors.signalTrendType || signal.signalTrendType || '';
        const smCount = signal.smart_wallet_online || analysis?.token?.smartWalletOnline || 0;

        if (tier !== 'GOLD' && signalType === 'STABLE' && score >= 58 && smCount >= 2) {
            console.log(`   🛡️ [STABLE快速通道] ${signal.symbol} 触发金狗保护: STABLE + score=${score} + SM=${smCount}`);
            tier = 'GOLD';
        }

        // v9.0 ACCELERATING 信号强制保护
        // ACCELERATING 信号代表链上加速，优先进入金池
        if (tier !== 'GOLD' && signalType === 'ACCELERATING' && score >= 50) {
            console.log(`   🔥 [ACCELERATING快速通道] ${signal.symbol} 链上加速信号: score=${score}`);
            tier = 'GOLD';
        }

        // v9.0 SM=2 黄金组合保护
        // SM=2 是最理想的聪明钱数量（不多不少），优先进入银池以上
        if (tier === 'BRONZE' && smCount === 2 && score >= 50) {
            console.log(`   ⭐ [SM=2黄金组合] ${signal.symbol} 升级到银池: SM=2 + score=${score}`);
            tier = 'SILVER';
        }

        // 2. 检查容量 & 执行挤压
        const canEnter = this.ensureCapacity(tier, score);
        if (!canEnter) {
    ...
    promote(record, newTier, reason) {
        ...
        if (canPromote) {
            ...
            // v7.4: 发射晋级事件，通知 BatchAIAdvisor 快轨分析
            if (newTier === 'GOLD') {
                this.emit('promotion', {
                    token: record,
                    fromTier: oldTier,
    ...
    async executeGraduation(record, factors, observeMinutes, graduationType) {
        ...
        if (useAI) {
            try {
                ...
                // ═══════════════════════════════════════════════════════════════
                try {
                    snapshotRecorder.recordAIDecision(
                        {
                            tokenAddress: record.address,
                            chain: record.chain,
                            symbol: record.symbol,
                            currentPrice: currentData?.current_price || record.initial?.current_price,
                            marketCap: currentData?.market_cap || record.initial?.market_cap,
                            liquidity: currentData?.liquidity_usd || record.initial?.liquidity_usd,
                            smartMoney: currentData?.smart_wallet_online || record.initial?.smart_wallet_online,
                            signalSource: record.signalSource || 'tiered_observer'
                        },
                        {
                            action: decision.action,
                            reason: decision.reason,
                            position: decision.position,
                            narrativeTier: decision.intention_tier,
                            narrativeReason: decision.intention_reason,
                            targetMcap: decision.target_mcap,
                            confidence: decision.confidence || 50
                        },
                        { signalSource: 'tiered_observer', tier: record.tier }
                    );
                } catch (recordErr) {
                    // 静默处理，不影响主流程
                }
