...
def test_mode_gate_scope_audit_reports_final_normal_tiny_scope_covered():
    ...
    assert "SecretsManagementContract" not in normal["missing_contracts"]
    ...
    assert "CapacityPlanningContract" not in normal["missing_contracts"]
    assert "LoadTestReplayContract" not in normal["missing_contracts"]
    assert "LatencyAttributionContract" not in normal["missing_contracts"]
    assert "ProviderQuotaIsolationContract" not in normal["missing_contracts"]
    assert "EconomicCostBudgetContract" not in normal["missing_contracts"]
    assert "SystemSLO" not in normal["missing_contracts"]
    assert "BackpressurePolicy" not in normal["missing_contracts"]
    assert "BudgetReserveContract" not in normal["missing_contracts"]
    assert "OperatorAudit" not in normal["missing_contracts"]
    assert "OperatorSafetyContract" not in normal["missing_contracts"]
    assert "OwnershipOnCallContract" not in normal["missing_contracts"]
    assert "AlertPolicy" not in normal["missing_contracts"]
    assert "AlertAckEscalationPolicy" not in normal["missing_contracts"]
    assert "KillSwitchDrillContract" not in normal["missing_contracts"]
    assert "ReplayDeterminismCheck" not in normal["missing_contracts"]
    assert "ReproducibleBuildContract" not in normal["missing_contracts"]
    assert "SupplyChainSecurityContract" not in normal["missing_contracts"]
    assert "PolicyBundleCompatibilityContract" not in normal["missing_contracts"]
    assert "ModelExpiryContract" not in normal["missing_contracts"]
    assert "ForecastSanityGuard" not in normal["missing_contracts"]
    assert "TelegramLifecycleTransitionMatrixContract" not in shadow["missing_contracts"]
    assert "LifecycleNstepForecastContract" not in shadow["missing_contracts"]
    assert "AbsorbingSemiMarkovForecastContract" not in shadow["missing_contracts"]
    assert "CompetingRiskForecastContract" not in shadow["missing_contracts"]
    assert "CensoringPolicyContract" not in shadow["missing_contracts"]
    assert "ForecastWalkForwardValidationContract" not in shadow["missing_contracts"]
    assert "HMMResearchOnlyBoundaryContract" not in shadow["missing_contracts"]
    assert "RenderedSpecViewContract" not in normal["missing_contracts"]
    assert "HealthStateEnumContract" not in normal["missing_contracts"]
    assert "ContractLifecycleContract" not in normal["missing_contracts"]
    assert "ObjectivePriorityContract" not in normal["missing_contracts"]
    assert "TokenIdentityContract" not in normal["missing_contracts"]
    assert "DataUnitContract" not in normal["missing_contracts"]
    assert "ChainFinalityContract" not in normal["missing_contracts"]
    assert "ProviderSchemaContract" not in normal["missing_contracts"]
    assert "LifecycleStateMachineContract" not in normal["missing_contracts"]
    assert "ExitExecutionStateMachine" not in normal["missing_contracts"]
    assert "ExitPolicyContract" not in normal["missing_contracts"]
    assert "CircuitBreakerPositionPolicy" not in normal["missing_contracts"]
    assert "EmergencyExitJournal" not in normal["missing_contracts"]
    assert "ExitQueueHealthContract" not in normal["missing_contracts"]
    assert "ReconciliationPolicyContract" not in normal["missing_contracts"]
    assert "DashboardStalenessContract" not in normal["missing_contracts"]
    assert "SpecTraceabilityMatrix" not in normal["missing_contracts"]
    assert "ImplementationIssueGraphContract" not in normal["missing_contracts"]
    assert "ModuleClosureContract" not in normal["missing_contracts"]
    assert "DecommissionPolicyContract" not in normal["missing_contracts"]
    assert "GoalConfidenceContract" not in normal["missing_contracts"]
    assert "FillTimeAnchorContract" not in normal["missing_contracts"]
    assert "ExAnteVsPosthocFeasibilityContract" not in normal["missing_contracts"]
    ...
    assert "NoTradeRootCause" not in ultra["missing_contracts"]
    ...
    assert "BlindedHoldoutContract" not in normal["missing_contracts"]
    assert "ManualOverrideContract" not in normal["missing_contracts"]
    assert "ReleaseComplexityBudget" not in normal["missing_contracts"]
    assert "ContractTestSuite" not in normal["missing_contracts"]
    assert "AdversarialReplaySuite" not in normal["missing_contracts"]
